...
HP_SIGNAL_QTY = 5
HP_SIGNAL_EXTREME_TOL = 2.0
HP_SIGNAL_MAX_AGE = 70
HP_SIGNAL_ENTRY_Z = 0.75
HP_SIGNAL_EDGE = 1.5
HP_SIGNAL_ORDER_SIZE = 8
...
def update_hp_market_signal(
    ...
    if market_trades and trade_key != signal.get("last_trade_key"):
        ...
        elif qty5_sells >= 1 and near_high:
            active = -1
            age = 0
            anchor = float(prev_mid)
...
def hp_orders(state: Dict[str, Any], trading_state: TradingState) -> List[Order]:
    ...
    signal_label = f"flow={signal_active}_age={signal_age}"
    ...
    if long_exit:
    ...
    elif bullish_flow and buy_z <= -HP_SIGNAL_ENTRY_Z and buy_momentum_ok and buy_edge > HP_SIGNAL_EDGE and position < 0.60 * POSITION_LIMITS[HP]:
        desired = HP_SIGNAL_ORDER_SIZE
        size = position_safe_buy_size(HP, desired, position, POSITION_LIMITS[HP], HP_PRACTICAL_CAP)
        reason = f"HP_BUY_market_extreme_resid_z={buy_z:.2f}_std={rolling_std:.2f}_{signal_label}_ret_ema={return_ema:.2f}_pred_move={prediction['predicted_move']:.2f}"
        log_decision(state, trading_state.timestamp, HP, adjusted, best_bid, best_ask, buy_edge, position, size, reason)
        if size > 0:
            orders.append(Order(HP, best_ask, size))
    elif bearish_flow and sell_z >= HP_SIGNAL_ENTRY_Z and sell_momentum_ok and sell_edge > HP_SIGNAL_EDGE and position > -0.60 * POSITION_LIMITS[HP]:
        desired = HP_SIGNAL_ORDER_SIZE
        size = position_safe_sell_size(HP, desired, position, POSITION_LIMITS[HP], HP_PRACTICAL_CAP)
        reason = f"HP_SELL_market_extreme_resid_z={sell_z:.2f}_std={rolling_std:.2f}_{signal_label}_ret_ema={return_ema:.2f}_pred_move={prediction['predicted_move']:.2f}"
        log_decision(state, trading_state.timestamp, HP, adjusted, best_bid, best_ask, sell_edge, position, -size, reason)
        if size > 0:
            orders.append(Order(HP, best_bid, -size))
    elif best_ask < lower_band and buy_z <= -HP_ENTRY_Z and buy_momentum_ok and buy_edge > buy_threshold and position < 0.70 * POSITION_LIMITS[HP]:
        ...
        if size > 0:
            orders.append(Order(HP, best_ask, size))
    elif best_bid > upper_band and sell_z >= HP_ENTRY_Z and sell_momentum_ok and sell_edge > sell_threshold and position > -0.70 * POSITION_LIMITS[HP]:
        desired = HP_MAX_ORDER_SIZE

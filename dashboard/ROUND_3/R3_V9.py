...
def hp_orders(state: Dict[str, Any], trading_state: TradingState) -> List[Order]:
    ...
    if long_exit:
    ...
    elif HP_PASSIVE_SIGNAL_ORDERS and bullish_flow and position < 0.60 * POSITION_LIMITS[HP]:
        ...
        if passive_edge >= HP_PASSIVE_MAX_NEGATIVE_EDGE and size > 0:
            reason = f"HP_BUY_passive_signal_resid_z={buy_z:.2f}_std={rolling_std:.2f}_{signal_label}_edge={passive_edge:.2f}_ret_ema={return_ema:.2f}"
            ...
            reason = hp_signal_skip_reason("BUY", signal_active, signal_age, passive_edge, position)
            log_decision(state, trading_state.timestamp, HP, adjusted, best_bid, best_ask, passive_edge, position, 0, reason)
    elif bullish_flow and buy_edge > HP_SIGNAL_EDGE and position < 0.60 * POSITION_LIMITS[HP]:

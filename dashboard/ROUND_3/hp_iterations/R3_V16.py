...
HP_COPY_ORDER_SIZE = 8
HP_COPY_CROSS_MAX_AGE = 60
HP_COPY_MAX_RICH_Z = 2.50
HP_COPY_MIN_EDGE = -8.0
...
def hp_orders(state: Dict[str, Any], trading_state: TradingState) -> List[Order]:
    ...
    copy_bad_block = bad_buy_age <= HP_COPY_BAD_BLOCK_AGE
    copy_bullish_flow = (
        bullish_flow
    ...
    upward_mean_reversion = (
        (bullish_flow or secondary_bullish_flow or capitulation_bullish_flow)
        and not bad_buy_active
        and buy_z <= HP_SIGNAL_MAX_RICH_Z
    )
    downward_mean_reversion = market_downtick and sell_z >= HP_DOWNTICK_MIN_Z
    signal_label = f"flow={signal_active}_age={signal_age}_combo_age={combo_age}_cap_age={capitulation_age}_bad_buy_age={bad_buy_age}_down_age={downtick_age}"
    ...
    if copy_long_signal and position < HP_COPY_LONG_CAP:
        desired = HP_COPY_ORDER_SIZE
        if capitulation_bullish_flow or mean_reversion_good:
            desired = int(HP_COPY_ORDER_SIZE * 1.5)
        size = position_safe_buy_size(HP, desired, position, POSITION_LIMITS[HP], HP_COPY_LONG_CAP)
        reason = f"HP_COPY_BUY_signal_z={buy_z:.2f}_std={rolling_std:.2f}_copy_age={copy_signal_age}_{signal_label}_edge={buy_edge:.2f}_ret_ema={return_ema:.2f}"
        ...
        reason = f"HP_REDUCE_LONG_down_reversion_sell_z={sell_z:.2f}_std={rolling_std:.2f}_{signal_label}_edge={sell_edge:.2f}_ret_ema={return_ema:.2f}"
        ...
        if size > 0:
            orders.append(Order(HP, best_ask, size))
    elif HP_PASSIVE_SIGNAL_ORDERS and upward_mean_reversion and position < HP_LONG_ADD_CAP:
    ...
    elif upward_mean_reversion and buy_edge > HP_SIGNAL_EDGE and position < HP_LONG_ADD_CAP:

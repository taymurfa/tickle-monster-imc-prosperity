...
Current mode: HP-only V38 with small falling-knife probe buys.
...
HP_PROBE_BUY_ENABLED = True
HP_PROBE_MIN_CONFIDENCE = 8
HP_PROBE_MAX_Z = -1.50
HP_PROBE_MIN_EDGE = 1.0
HP_PROBE_MAX_NEGATIVE_RETURN = -2.0
HP_PROBE_ORDER_SIZE = 6
HP_PROBE_CAP = 24
...
def hp_orders(state: Dict[str, Any], trading_state: TradingState) -> List[Order]:
    ...
    probe_buy_signal = (
        HP_PROBE_BUY_ENABLED
        and not new_buy_block
        and not net_exhaustion_block
        and buy_confidence >= HP_PROBE_MIN_CONFIDENCE
        and buy_z <= HP_PROBE_MAX_Z
        and buy_edge >= HP_PROBE_MIN_EDGE
        and return_ema >= HP_PROBE_MAX_NEGATIVE_RETURN
        and position < HP_PROBE_CAP
    )
    ...
    elif probe_buy_signal:
        desired = HP_PROBE_ORDER_SIZE
        size = position_safe_buy_size(HP, desired, position, POSITION_LIMITS[HP], HP_PROBE_CAP)
        reason = f"HP_PROBE_BUY_falling_knife_z={buy_z:.2f}_std={rolling_std:.2f}_{signal_label}_edge={buy_order_edge:.2f}_ret_ema={return_ema:.2f}_pred_move={prediction['predicted_move']:.2f}_buy_px={buy_order_price}"
        log_decision(state, trading_state.timestamp, HP, adjusted, best_bid, best_ask, buy_order_edge, position, size, reason)
        if size > 0:
            orders.append(Order(HP, buy_order_price, size))

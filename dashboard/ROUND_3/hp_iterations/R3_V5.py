...
def hp_orders(state: Dict[str, Any], trading_state: TradingState) -> List[Order]:
    ...
    long_exit = position > 0 and sell_z >= -HP_EXIT_Z
    short_exit = position < 0 and buy_z <= HP_EXIT_Z
    ...
    if long_exit:
        ...
        reason = f"HP_EXIT_LONG_resid_z={sell_z:.2f}_std={rolling_std:.2f}_ret_ema={return_ema:.2f}_pred_move={prediction['predicted_move']:.2f}"
        ...
        size = min(size, -position)
        reason = f"HP_EXIT_SHORT_resid_z={buy_z:.2f}_std={rolling_std:.2f}_ret_ema={return_ema:.2f}_pred_move={prediction['predicted_move']:.2f}"
        ...
        size = position_safe_buy_size(HP, desired, position, POSITION_LIMITS[HP], HP_PRACTICAL_CAP)
        reason = f"HP_BUY_resid_z={buy_z:.2f}_std={rolling_std:.2f}_ret_ema={return_ema:.2f}_pred_move={prediction['predicted_move']:.2f}"
        log_decision(state, trading_state.timestamp, HP, adjusted, best_bid, best_ask, buy_edge, position, size, reason)
        if size > 0:
            orders.append(Order(HP, best_ask, size))
        ...
        if sell_edge >= HP_STRONG_EDGE or sell_z >= HP_STRONG_Z:
            desired = min(2 * HP_MAX_ORDER_SIZE, 24)
        size = position_safe_sell_size(HP, desired, position, POSITION_LIMITS[HP], HP_PRACTICAL_CAP)
        reason = f"HP_SELL_resid_z={sell_z:.2f}_std={rolling_std:.2f}_ret_ema={return_ema:.2f}_pred_move={prediction['predicted_move']:.2f}"
        log_decision(state, trading_state.timestamp, HP, adjusted, best_bid, best_ask, sell_edge, position, -size, reason)
        if size > 0:
            orders.append(Order(HP, best_bid, -size))
    else:
        ...
        reason = f"HP_NO_TRADE_resid_z={z:.2f}_std={rolling_std:.2f}_ret_ema={return_ema:.2f}_pred_move={prediction['predicted_move']:.2f}"

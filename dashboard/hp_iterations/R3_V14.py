...
def hp_orders(state: Dict[str, Any], trading_state: TradingState) -> List[Order]:
    ...
    upward_mean_reversion = bullish_flow and not bad_buy_active and buy_z <= HP_SIGNAL_MAX_RICH_Z
    downward_mean_reversion = market_downtick and sell_z >= HP_DOWNTICK_MIN_Z
    signal_label = f"flow={signal_active}_age={signal_age}_bad_buy_age={bad_buy_age}_down_age={downtick_age}"

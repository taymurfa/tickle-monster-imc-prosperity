"""
IMC Prosperity Round 3 combined strategy.

Active layers:
1. HYDROGEL_PACK: locked V42 confidence-scaled mean reversion with selective
   post-profit re-entry and protected late-day cleanup.
2. VELVETFRUIT_EXTRACT: VFE_VEV_V1 contrarian market making with imbalance-gated
   inside-spread aggression.
3. VEV vouchers: VFE_VEV_V1 empirical-premium market making and monotonicity arb.

The HP layer is intentionally copied from R3_V42 without parameter tuning. The
VFE/VEV layers are intentionally copied from VFE_VEV_V1.
"""
...
VFE_TAKER_ENABLED = True
VFE_TAKER_THRESHOLD = 6.0
VFE_TAKER_SIZE = 4
VFE_TAKER_CAP = 80
VFE_TAKER_INVENTORY_PENALTY = 0.02
...
def load_state(blob: str) -> Dict[str, Any]:
    ...
    if not isinstance(default["vfe_stats"], dict):
        default["vfe_stats"] = {}
    if not isinstance(default["voucher_premium_ema"], dict):
        default["voucher_premium_ema"] = {}
...
def vfe_orders(state: Dict[str, Any], trading_state: TradingState, fair: float) -> List[Order]:
    ...
    adjusted_fair = fair - VFE_TAKER_INVENTORY_PENALTY * position

    # Accurate backtests with matching_mode="none" only fill taker orders.
    # The old VFE layer was passive-only, so it showed zero PnL in that mode.
    # This conservative taker layer only crosses for large EMA dislocations.
    if VFE_TAKER_ENABLED:
        if best_ask < adjusted_fair - VFE_TAKER_THRESHOLD and position < VFE_TAKER_CAP:
            room = min(VFE_TAKER_CAP - position, POSITION_LIMITS[VFE] - position)
            size = min(VFE_TAKER_SIZE, ask_vol, max(0, room))
            if size > 0:
                orders.append(Order(VFE, best_ask, size))
                log_decision(
                    state,
                    ...
                    best_bid,
                    best_ask,
                    adjusted_fair - best_ask,
                    position,
                    size,
                    "VFE_TAKER_BUY_EMA_REVERSION",
                )

        if best_bid > adjusted_fair + VFE_TAKER_THRESHOLD and position > -VFE_TAKER_CAP:
            room = min(VFE_TAKER_CAP + position, POSITION_LIMITS[VFE] + position)
            size = min(VFE_TAKER_SIZE, bid_vol, max(0, room))
            if size > 0:
                orders.append(Order(VFE, best_bid, -size))
                log_decision(
                    state,
                    trading_state.timestamp,
                    VFE,
                    ...
                    best_bid,
                    best_ask,
                    best_bid - adjusted_fair,
                    position,
                    -size,
                    "VFE_TAKER_SELL_EMA_REVERSION",
                )
